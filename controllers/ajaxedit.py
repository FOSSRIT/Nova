...
def watch():
    ...
    if add_rem_watch(node, True):
        response.view = "generic.load"
        return A(SPAN(SPAN(_class="heart icon"),'Unwatch'),_class="middle negative button",_href=URL('ajaxedit','unwatch',args=node.url), cid=the_cid)
...
def unwatch():
    ...
    if add_rem_watch(node, False):
        response.view = "generic.load"
        return A(SPAN(SPAN(_class="heart icon"),'Watch'),_class="middle button", _href=URL('ajaxedit','watch',args=node.url), cid=the_cid)
    else:
